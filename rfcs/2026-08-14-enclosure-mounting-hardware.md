...
though a process view would sequence them at final assembly, once the board is in
the box. Part 5 shows the sequence is derivable from what the solver already
computes, so ordering is a projection rather than a second tree.
...
#### 1.5.1 It is a second hierarchy, not the existing one
...
position at all. They are separate trees because they legitimately disagree about
their own shape.
...
- **The root is the device.** A tree of parts needs no separate device record: the
...
- **`manufacturing_process` sits on the part being made**, not on the assembly
  ...
  is not.
- **A subassembly carries no geometry.** A renderer given both a subassembly's
  mesh and its children's would draw the enclosure twice, and the two drift as
  soon as a child changes. Geometry lives on leaves; an assembled preview is a
  union of them.
...
**The specification is the identity and the part number is not** -- the inverse of
the electrical BOM's rule, following from §1.3: any screw meeting `ISO 4762 M3x8`
will do, so two of them are one line whatever their part numbers say.

The argument is stability. A part number is the *result of a lookup* -- against a
vendor catalogue, at a moment, possibly over a network -- so keying on it makes
the **shape of the BOM depend on that lookup**: the same unchanged design,
resolved on a day one vendor is out of stock, splits one line of eight into two of
four.

So `designation`, `display_value`, `manufacturer_part_number` and
`supplier_part_numbers` are not four competing identities: the designation
identifies, the display value is what a human reads, and the last two are
*sourcing* -- the resolved answer cached into the artifact so it can be ordered
without re-running an engine, exactly as `source_component` caches what
`partsEngine` resolved. Where two pieces genuinely must be bought separately, a
different material or finish, that difference belongs in the specification;
material and finish are not axes yet (§2.1).
...
accommodate hardware. The alternative -- a free-standing column in a corner --
stands tangent to two inside walls and reaches diagonally into the space the board
wants: measured, an M3 column takes a 46 x 30 box to 57.7 x 41.7, and needs a
whole XY dimension-inference rule to pay for it.

The cost is that placement no longer selects the mount kind on its own: both
reaches are declared in a hole, so `fastens` has to say which. In exchange no boss
exists that is not anchored to a hole, so none can want floor area the board has
not already cleared.
...
on a prototype fixture predating the corner column's removal, so the operation
count differs from the example above; the derivation does not):
